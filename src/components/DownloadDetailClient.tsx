...
import { quranJuzData } from "@/data/quranJuzData";
import { englishTranslationJuzData } from "@/data/englishTranslationJuzData";
import {
  ArrowLeft,
  Download,
  BookOpen,
...
export default function DownloadDetailClient({ item }: DownloadDetailClientProps) {
  const [activeModalItem, setActiveModalItem] = useState<{ pdfUrl: string; title: string } | null>(null);
  const [downloadedId, setDownloadedId] = useState<string | null>(null);

  const isTajweediQuranPage = item.slug === "colorful-tajweedi-quran";
  const isEnglishTranslationPage = item.slug === "english-quran-word-to-word-translation";
  ...
  // 1. Tajweedi Quran 31 Compact Boxes View
  ...
  // 2. English Quran Word to Word Translation 30 Compact Boxes View
  ...
  // Fallback view for single download items
